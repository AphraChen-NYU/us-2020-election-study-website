...
import type { StudyDataset } from "@/data/study-datasets";

interface DatasetCitationDialogProps {
  dataset: StudyDataset | null;
  onClose: () => void;
}

export function DatasetCitationDialog({ dataset, onClose }: DatasetCitationDialogProps) {
  const titleId = useId();
  ...
  const [citationBeforeDoi, citationAfterDoi = ""] = dataset.citation.split(dataset.doi);
      ...
      >
        <header className="flex items-center justify-between gap-6 border-b border-[#14213d]/12 px-5 py-5 sm:px-8 sm:py-6">
          ...
          <button
            ...
            type="button"
            onClick={onClose}
            aria-label="Close dataset citation"
